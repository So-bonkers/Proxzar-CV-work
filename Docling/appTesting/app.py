...
def process_document(file_path, output_dir):
    """
    Process the document using Docling, extracting text and figures.
    Save extracted content as JSON and figures as PNG.
    """
    try:
        start_time = time.time()

        # Configure pipeline options for figures
        ...
        # Save only figures as PNG
        figure_counter = 0
        for element, _ in conv_result.document.iterate_items():
            if isinstance(element, PictureItem):  # Save only PictureItems (figures)
                figure_counter += 1
        ...
        return {
            "message": f"File processed successfully with {figure_counter} figures saved.",
